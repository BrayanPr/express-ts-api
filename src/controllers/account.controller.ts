...
export default class AccountController {
  ...
  async getAllAccounts(): Promise<SuccessResponse | ErrorResponse> {
    return await Account.findAll()
      .then((accounts: Array<Account>) => FoundResponse(accounts, "Accounts"))
      .catch((err: any) =>
        ServerErrorResponse(
          "Error while consulting accounts",
  ...
  async createAccount(body: any): Promise<SuccessResponse | ErrorResponse> {
    ...
    return Account.create(account)
      .then((instance: Account) => CreatedResponse(instance, "Account"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
          "Error while creating account" + JSON.stringify(body)
  ...
  async updateAccount(
    ...
    if (old_account == null) return NotFoundResponse(id, "Account");
    else accountDtoErrors = toAccountUpdateDto(body);
    ...
    return await Account.update(account, { where: { id: id } })
      .then(async () => UpdatedResponse(true, "Account"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
  ...
  async deleteAccount(id: number): Promise<SuccessResponse | ErrorResponse> {
    ...
    if (account == null) return NotFoundResponse(id, "Account");

    return await Account.destroy({ where: { id: id } })
      .then(() => DeletedResponse(true, "Account"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
          `Error while deleting account with id: ${id}`
        )
      );
  }
  async getAccountById(id: number): Promise<SuccessResponse | ErrorResponse> {
    let instance = Account.findByPk(id);
    if (instance == null) return NotFoundResponse(id, "Account");
