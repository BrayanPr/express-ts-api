import { SuccessResponse, HistoryDTO, User as IUser } from "../../types";
import {
  BadRequestResponse,
  FoundResponse,
  ServerErrorResponse,
} from "../responses";
import { sequelizeErrorHandler, toHistoryDto } from "../utils";
...
export default class OperationController {
  ...
  async moveUser(body: any) {
    ...
    if (teamQuery.statusCode != 200)
      if ("error" in teamQuery) return teamQuery;
      else
        return ServerErrorResponse(
          "Unknown error",
          "Error while moving user: " + JSON.stringify(body)
        );

    let userQuery = await this.userController.getUserById(historyDto.user);

    if (userQuery.statusCode != 200)
      if ("error" in userQuery) return userQuery;
      else
        return ServerErrorResponse(
          "Unknown error",
          "Error while moving user: " + JSON.stringify(body)
    ...
    if ("instance" in userQuery) user = userQuery.instance;
    else
      return ServerErrorResponse(
        "Unknown error",
        "Error while consulting user: " + JSON.stringify(body)
      );

    if (user.teamId == historyDto.teamJoin)
      return BadRequestResponse(
        "Error while moving user",
        `User:${user.id} is already on team ${user.teamId}`
      );
    ...
    if (!userUpdated)
      return ServerErrorResponse(
        ...
        sequelizeErrorHandler(moveUserError)
      );
  ...
  async getAllHistory() {
    return await History.findAll()
      .then((instance: Array<History>) => FoundResponse(instance, "History"))
      .catch((err) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
          "Error while consulting users"
        )
      );
  }
}
