...
export default class UserController {
  ...
  async getAllUsers(): Promise<SuccessResponse | ErrorResponse> {
    return await User.findAll()
      .then((instance: Array<User>) => FoundResponse(instance, "Users"))
      .catch((err: any) =>
        ServerErrorResponse(
  ...
  async updateUser(
    ...
    if (old_user == null) NotFoundResponse(id, "User");
  ...
  async createUser(body: any): Promise<SuccessResponse | ErrorResponse> {
    ...
    return await User.create(user)
      .then((instance: User) => CreatedResponse(instance, "User"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
          "Error while creating user: " + JSON.stringify(user)
        )
      );
  }
  async createUserAdmin(body: any): Promise<SuccessResponse | ErrorResponse> {
    ...
    user.role = "admin";
    return await User.create(user)
      .then((instance: User) => CreatedResponse(instance, "User admin"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
          "Error while creating user: " + JSON.stringify(user)
  ...
  async deleteUser(id: number): Promise<SuccessResponse | ErrorResponse> {
    ...
    if (user == null) return NotFoundResponse(id, "User");

    return await User.destroy({ where: { id: id } })
      .then(() => DeletedResponse(true, "User"))
      .catch((err: any) =>
        ServerErrorResponse(
          sequelizeErrorHandler(err),
  ...
  async getUserById(id: number): Promise<SuccessResponse | ErrorResponse> {
    ...
    if (instance == null) return NotFoundResponse(id, "User");
  ...
  async Auth(body: any): Promise<ErrorResponse | SuccessResponse> {
    ...
    return await User.findOne({
      ...
      .then(async (user: User | null) => {
        if (user == null) return BadCredentialsResponse();
      ...
      .catch((err: any) => ServerErrorResponse(err, "Error while login in"));
  }
}
